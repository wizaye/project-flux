...
import { AgentActivity } from "#components/agents/agent-activity/index";
...
const ThreadRoot: FC<{
  composerControls?: ReactNode;
  composerContext?: ReactNode;
}> = ({ composerControls, composerContext }) => {
      ...
      >
        ...
        >
          ...
          <ThreadPrimitive.ViewportFooter
            className="aui-thread-viewport-footer sticky bottom-0 mt-auto flex flex-col gap-2 overflow-visible bg-transparent pb-2"
          >
            <ThreadScrollToBottom />
            <ThreadFollowupSuggestions />
            <Composer controls={composerControls} context={composerContext} />
...
const ThreadWelcome: FC = () => {
  return (
    <div className="aui-thread-welcome-root flex flex-1 flex-col items-center justify-center gap-1 px-4 py-8 text-center">
      <h1 className="aui-thread-welcome-message-inner text-base font-medium tracking-tight">
        What would you like to build?
      </h1>
      <p className="max-w-[34ch] text-xs leading-5 text-muted-foreground">
        Ask about the workspace, plan a change, or let the agent work with files.
      </p>
    </div>
  );
};
...
const Composer: FC<{ controls?: ReactNode; context?: ReactNode }> = ({ controls, context }) => {
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone render={<div data-slot="aui_composer-shell" className="border-border/60 data-[dragging=true]:border-ring focus-within:border-foreground/35 flex w-full cursor-text flex-col gap-2 rounded-xl border bg-(--composer-bg) p-(--composer-padding) transition-[border-color] data-[dragging=true]:border-dashed data-[dragging=true]:bg-accent/50" />}><ComposerAttachments /><ComposerPrimitive.Input
                      placeholder="Describe what to build"
                      className="aui-composer-input caret-primary placeholder:text-muted-foreground/60 max-h-48 min-h-14 w-full resize-none bg-transparent px-2 py-1 text-sm leading-5 outline-none"
                      rows={1}
                      autoFocus
                      enterKeyHint="send"
                      aria-label="Message input"
                    /><ComposerAction controls={controls} context={context} /></ComposerPrimitive.AttachmentDropzone>
    </ComposerPrimitive.Root>
  );
};

const ComposerAction: FC<{ controls?: ReactNode; context?: ReactNode }> = ({ controls, context }) => {
  return (
    <div className="aui-composer-action-wrapper relative flex min-w-0 flex-col gap-1">
  ...
  return (
    ...
    >
          ...
          {({ part, children }) => {
            switch (part.type) {
              ...
              case "group-reasoning": {
                ...
                return (
                  <AgentActivity
                    items={[{ id: "reasoning", type: "text", content: children }]}
      ...
      <div
        ...
        className={cn("ms-2 flex flex-wrap items-center gap-x-2", ACTION_BAR_HEIGHT)}
      >
        <BranchPicker />
        <AssistantActionBar />
        <MessageMetrics />
...
const MessageMetrics: FC = () => {
  const timing = useAuiState((state) => state.message.metadata?.timing);
  const custom = useAuiState((state) => state.message.metadata?.custom);
  const createdAt = useAuiState((state) => state.message.createdAt);
  const running = useAuiState((state) => state.message.status?.type === "running");

  const model = typeof custom?.modelLabel === "string" ? custom.modelLabel : undefined;
  if (running || (!timing?.tokenCount && !model)) return null;
  const seconds = timing?.totalStreamTime ? timing.totalStreamTime / 1000 : undefined;

  return (
    <span
      className="ms-auto text-[10px] leading-5 text-muted-foreground tabular-nums"
      title="Response usage"
    >
      {createdAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
      {seconds !== undefined ? ` · ${seconds.toFixed(1)}s` : ""}
      {timing?.tokenCount ? ` · ${timing.tokenCount.toLocaleString()} tokens` : ""}
      {model ? ` · ${model}` : ""}
    </span>
  );
};
