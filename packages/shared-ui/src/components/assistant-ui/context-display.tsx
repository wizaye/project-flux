...
function ContextDisplayTrigger({
  ...
  return (
    <TooltipTrigger render={<button type="button" data-slot="context-display-trigger" className={cn(
                "inline-flex items-center rounded-md transition-colors",
                className,
              )} {...props} />}>{children}</TooltipTrigger>
  );
}
...
function ContextDisplayContent({
  ...
  return (
    <TooltipContent
      ...
      className={cn(
        "bg-popover text-popover-foreground w-max min-w-48 rounded-lg border p-3 text-left [&_[data-slot=tooltip-arrow]]:hidden",
        className,
      )}
    >
...
const ContextDisplayRing: FC<PresetProps> = ({
  ...
  <ContextDisplayRoot modelContextWindow={modelContextWindow} usage={usage}>
    ...
    >
      <RingVisual />
      {showPercent ? <RingPercentLabel /> : null}
    </ContextDisplayTrigger>
